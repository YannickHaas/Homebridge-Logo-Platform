...
exports.LightbulbPlatformAccessory = void 0;
...
class LightbulbPlatformAccessory {
    model = "Lightbulb";
    api;
    service;
    information;
    ...
    async setOn(value) {
        ...
        let qItem;
        if (value) {
            qItem = new queue_1.QueueSendItem(this.device.lightbulbSetOn, value, this.pushButton);
        }
        else {
            qItem = new queue_1.QueueSendItem(this.device.lightbulbSetOff, value, this.pushButton);
        }
    ...
    async setBrightness(value) {
        this.accStates.Brightness = value;
        if (this.platform.config.debugMsgLog || this.device.debugMsgLog) {
            this.platform.log.info('[%s] Set Brightness <- %i', this.device.name, value);
        }
        let qItem = new queue_1.QueueSendItem(this.device.lightbulbSetBrightness, value, 0);
        this.platform.queue.bequeue(qItem);
    }
    async getBrightness() {
    ...
    updateOn() {
        ...
        let qItem = new queue_1.QueueReceiveItem(this.device.lightbulbGet, async (value) => {
    ...
    updateBrightness() {
        if (this.updateBrightnessQueued) {
            return;
        }
        let qItem = new queue_1.QueueReceiveItem(this.device.lightbulbGetBrightness, async (value) => {
            if (value != error_1.ErrorNumber.noData) {
                this.accStates.Brightness = value;
                if (this.platform.config.debugMsgLog || this.device.debugMsgLog) {
    ...
    logAccessory() {
        if ((this.platform.loggerType == logger_1.LoggerType.InfluxDB) && this.platform.influxDB.isConfigured) {
            ...
            logItems.push(new logger_1.InfluxDBLogItem("Brightness", this.accStates.Brightness, logger_1.InfluxDBFild.Int));
...
exports.LightbulbPlatformAccessory = LightbulbPlatformAccessory;
//# sourceMappingURL=lightbulbPlatformAccessory.js.map